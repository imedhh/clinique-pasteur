...
export async function POST(req: Request) {
  try {
      ...
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        ...
        <div style="background: #f9fafb; padding: 24px; border: 1px solid #e5e7eb; border-top: none; border-radius: 0 0 12px 12px;">
          <table style="width: 100%; border-collapse: collapse;">
            <tr><td style="padding: 8px 0; color: #666; width: 100px;">Nom</td><td style="padding: 8px 0; font-weight: bold;">${data.nom || '-'} ${data.prenom || ''}</td></tr>
            <tr><td style="padding: 8px 0; color: #666;">Email</td><td style="padding: 8px 0;"><a href="mailto:${data.email}">${data.email || '-'}</a></td></tr>
            <tr><td style="padding: 8px 0; color: #666;">Téléphone</td><td style="padding: 8px 0;">${data.telephone || '-'}</td></tr>
            <tr><td style="padding: 8px 0; color: #666;">Sujet</td><td style="padding: 8px 0; font-weight: bold;">${data.sujet || '-'}</td></tr>
          </table>
    ...
    await transporter.sendMail({
      ...
      replyTo: data.email,
      subject: `Contact - ${data.sujet || 'Message'} - ${data.nom} ${data.prenom}`,
      html,
    })
